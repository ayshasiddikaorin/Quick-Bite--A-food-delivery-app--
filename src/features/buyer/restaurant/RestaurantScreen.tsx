    ...
    <View style={cardStyles.card}>
      ...
      <View style={cardStyles.info}>
        ...
        <View style={cardStyles.footer}>
          <View>
            {discount > 0 && (
              <Text style={cardStyles.originalPrice}>${item.price.toFixed(2)}</Text>
            )}
...
const RestaurantScreen: React.FC = () => {
  ...
  const filteredMenu =
    activeCategory === 'Popular'
      ? restaurant.menu.filter((m) => m.isPopular)
      : restaurant.menu.filter((m) => m.category === activeCategory);

  const handleAddToCart = useCallback(
    async (menuItem: RestaurantMenuItem & { price: number }) => {
      ...
      setCartQty((prev) => {
        ...
        const updated = { ...prev, [realId]: next };
      ...
      try {
        ...
        if (isRemove) {
          ...
          if (existingIdx >= 0) {
            ...
            const realItem = restaurant.menu.find((m) => m.id === realId)!;
            cart.push({
  ...
  return (
    <View style={styles.root}>
      ...
      {/* ── Floating condensed header (shows on scroll) ─────────────────── */}
      <Animated.View
      ...
      {/* ── Back / fav buttons (always visible) ─────────────────────────── */}
      <SafeAreaView style={styles.topButtons} edges={['top']} pointerEvents="box-none">
        ...
        >
          <Ionicons
            ...
            size={20}
            color={isFav ? Colors.badge : Colors.black}
          />
        </TouchableOpacity>
      </SafeAreaView>
      ...
      >
        {/* ── Cover image ───────────────────────────────────────────────── */}
        <Animated.View
          style={[styles.coverWrapper, { transform: [{ translateY: coverTranslate }] }]}
        >
        ...
        <View style={styles.infoCard}>
          ...
          {offerDiscount && offerDiscount > 0 && (
            <View style={styles.offerRibbon}>
              ...
              <Text style={styles.offerRibbonText}>
                {offerDiscount}% OFF on all menu items
              </Text>
            </View>
          )}
          ...
          <View style={styles.infoRow}>
            ...
            <View style={[
              styles.openBadge,
              { backgroundColor: restaurant.isOpen ? Colors.successLight : Colors.errorLight },
            ]}>
              <View style={[
                styles.openDot,
...
const styles = StyleSheet.create({
  ...
  stickyHeader: {
    ...
    backgroundColor: Colors.white,
    paddingTop: 48, paddingBottom: 14, paddingHorizontal: 60,
    alignItems: 'center',
